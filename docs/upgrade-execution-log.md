...
- Scope:
  - Wave 4 graph filter main-sync
  - bring PR #13 onto the `main` branch after PR #9 extension surface landed
- Planned touchpoints:
  - `src-tauri/src/lib.rs`
  - `src/components/Sidebar/KnowledgeGraphPanel.tsx`
  - `src/components/Sidebar/KnowledgeGraphPanel.css`
  ...
  - `scripts/iteration-goal-audit.mjs`
- Product management baseline:
  - PR #13 now carries Wave 3 release evidence, PR #9 readonly plugin API contracts, and the hotfix9 release audit.
  - The merge queue marks PR #9 as merged and PR #13 as review-gated after main-sync.
  - The graph panel filter slice remains focused on folder, tag, search, and link-depth narrowing for local knowledge graph sensemaking.
- Verification target:
  - `npm run typecheck`
  - `npx playwright test tests/e2e_graph_panel.spec.ts --reporter=line`
  - `cargo check --manifest-path src-tauri/Cargo.toml`
